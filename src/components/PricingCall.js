...
const pricingCall = []
const localStorageDataName = "pricingCall"
let localStorageDataPull = JSON.parse(localStorage.getItem("pricingCall"))
if (localStorageDataPull == null) {
  localStorage.setItem(localStorageDataName, JSON.stringify(pricingCall))
  localStorageDataPull = JSON.parse(localStorage.getItem("pricingCall"))
}
...
function AddUser() {
  ...
  function submitNewCompany() {
    ...
    localStorage.setItem("pricingCall", JSON.stringify(oldObject))
    BookedDemoData()
  }
...
const RemoveUser = (id, companyName) => {
  if (window.confirm(`Are you sure you want to remove ${companyName}?`) === true) {
    const newpricingCall = localStorageDataPull.filter(
      (prospect) => prospect.id !== id)
    window.localStorage.setItem("pricingCall", JSON.stringify(newpricingCall))
    window.location.reload(true)
  }
}

function BookedDemoData() {
  let demoDataFromLocalStorage = localStorage.getItem("pricingCall")
  ...
  return (
    <div className="main-content">
      <div className="main-content-header">
        <h1>Prospects with a scheduled pricing call</h1>
