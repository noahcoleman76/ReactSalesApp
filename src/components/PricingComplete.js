...
const pricingCallCompleted = []
const localStorageDataName = "pricingCallCompleted"
let localStorageDataPull = JSON.parse(localStorage.getItem("pricingCallCompleted"))
if (localStorageDataPull == null) {
  localStorage.setItem(localStorageDataName, JSON.stringify(pricingCallCompleted))
  localStorageDataPull = JSON.parse(localStorage.getItem("pricingCallCompleted"))
}
...
function AddUser() {
  ...
  function submitNewCompany() {
    ...
    localStorage.setItem("pricingCallCompleted", JSON.stringify(oldObject))
    BookedDemoData()
  }
...
const RemoveUser = (id, companyName) => {
  if (window.confirm(`Are you sure you want to remove ${companyName}?`) === true) {
    const newpricingCallCompleted = localStorageDataPull.filter(
      (prospect) => prospect.id !== id)
    window.localStorage.setItem("pricingCallCompleted", JSON.stringify(newpricingCallCompleted))
    window.location.reload(true)
  }
}

function BookedDemoData() {
  let demoDataFromLocalStorage = localStorage.getItem("pricingCallCompleted")
  ...
  return (
    <div className="main-content">
      <div className="main-content-header">
        <h1>Prospects with a completed pricing call</h1>
...
export default PricingCallCompleted;