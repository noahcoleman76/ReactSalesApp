...
function AddUser() {
  ...
  function submitNewCompany() {
    ...
    localStorage.setItem("jumpStartCompleted", JSON.stringify(oldObject))
    BookedDemoData()
  }
...
const RemoveUser = (id, companyName) => {
  if (window.confirm(`Are you sure you want to remove ${companyName}?`) === true) {
    const newjumpStartCompleted = localStorageDataPull.filter(
      (prospect) => prospect.id !== id)
    window.localStorage.setItem("jumpStartCompleted", JSON.stringify(newjumpStartCompleted))
    window.location.reload(true)
  }
}

function BookedDemoData() {
  let demoDataFromLocalStorage = localStorage.getItem("jumpStartCompleted")
...
export default JumpStartCompleted;