...
const localStorageDataName = "completedDemos"
let localStorageDataPull = JSON.parse(localStorage.getItem("completedDemos"))
if (localStorageDataPull == null) {
  localStorage.setItem(localStorageDataName, JSON.stringify(completedDemos))
  localStorageDataPull = JSON.parse(localStorage.getItem("completedDemos"))
}
...
function AddUser() {
  ...
  function submitNewCompany() {
    ...
    localStorage.setItem("completedDemos", JSON.stringify(oldObject))
    BookedDemoData()
  }
...
const RemoveUser = (id, companyName) => {
  if (window.confirm(`Are you sure you want to remove ${companyName}?`) === true) {
    const newcompletedDemos = localStorageDataPull.filter(
      (prospect) => prospect.id !== id)
    window.localStorage.setItem("completedDemos", JSON.stringify(newcompletedDemos))
    window.location.reload(true)
  }
}

function BookedDemoData() {
  let demoDataFromLocalStorage = localStorage.getItem("completedDemos")
