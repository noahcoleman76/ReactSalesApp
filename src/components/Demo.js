...
function AddUser() {
  ...
  function submitNewCompany() {
    ...
    localStorage.setItem("bookedDemos", JSON.stringify(oldObject))
    BookedDemoData()
  }
...
const RemoveUser = (id, companyName) => {
  if (window.confirm(`Are you sure you want to remove ${companyName}?`) === true) {
    const newBookedDemos = localStorageDataPull.filter(
      (prospect) => prospect.id !== id)
    window.localStorage.setItem("bookedDemos", JSON.stringify(newBookedDemos))
    window.location.reload(true)
  }
}

function BookedDemoData() {
  let demoDataFromLocalStorage = localStorage.getItem("bookedDemos")
  ...
  return (
    <div className="main-content">
      <div className="main-content-header">
        <h1>Prospects with a booked demo</h1>
